...
bulochkaOutsideVideo = open('images/bul_out.mp4', 'rb')
cutieOutsideVideo = open('images/cutie_out.mp4', 'rb')
...
def bot_message(message):
    global bulochka_in, cutie_in, bulochka, bulochkaOutsideVideo, cutieOutsideVideo
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    item1 = types.KeyboardButton('Check status')
    ...
    if message.chat.type == 'private':
        if message.text == 'Check status' or message.text == 'Check status again':
            ...
            bot.send_message(message.chat.id, 'Bulochka\'s status was updated, Enjoy your walking, Bulochka!',
                             reply_markup=markup)
            bot.send_video(message.chat.id, bulochkaOutsideVideo)
            ...
            bot.send_message(message.chat.id, '\U0001F431 Cutie\'s status was updated, Enjoy your walking, Cutie!',
                             reply_markup=markup)
            bot.send_video(message.chat.id, cutieOutsideVideo)
